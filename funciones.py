...
def msg_cypher(msg: str) -> list:
    ...
    # Mensaje guardado como lista
    msg_list = list(msg_lower)

    # Adición de "-1" entre cada caracter
    for i in range(len(msg) - 1):
        msg_list.insert(i * 2 + 1,-1)
    
    # Suma de "1" para cada caracter
    for i, l in enumerate(msg_list):
        if l != -1:
            msg_list[i] = characters.index(l) + 1

    flag = False

    # Iteración para transformar cada lista de números en un solo número
    for i, l in enumerate(msg_list):
        if flag:
            flag = False
            continue
    ...
    # Agrega un "0" como último elemento de la lista
    msg_list.append(0)
        
    return msg_list

def encryptor(msg: str, path: str) -> Image.Image:
    '''
    ...
    # Iteración del largo del mensaje sobre pixeles de la imagen de dos en dos,
    for l in msg_c:
...
def decryptor(path: str) -> str:
    ...
    # Iteración sobre los pixeles de la imágen de dos en dos hasta encontrar un "0"
    ...
    # Uso de la función msg_decypher() para descifrar el mensaje desencriptado    
    msg = msg_decypher(msg_list)
...
def msg_decypher(msg_list: list) -> str:
    ...
    # Resta de "-1" a todos los caracteres
    ...
    while True:
        ...
        # Si el elemento es "0" (0 - 1), marca el final del mensaje
        if n == -1:
            if final_msg[-1] == []:
                final_msg.remove([])
            break
        
        # Si el elemento es "-1" (-1 - 1), marca una separación entre caracteres
        elif n == -2:
            i += 1
            j += 1
            final_msg += [[]]
            
        # Añade el caracter a la lista
        else:
            final_msg[j] += [n]
            i += 1

    # Iteración por cada elemento en la lista con los caracteres cifrados, para transformarlos en listas con números, a un número que representa el caracter
    ...
    # Iteración por cada caracter, reemplazandolo por cada caracter correspondiente
    for i in final_msg:
        msg.append(characters[i - 1])
